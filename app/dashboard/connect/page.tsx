...
// Импортируем модалки (убедись, что пути правильные)
import BotDetailsModal from "../components/BotDetailsModal";
import {PlatformDetailsModal} from "@/app/dashboard/page"; // Импортируем модалку для Telegram User
...
const Platforms = [
    {
        id: 'telegram', // ID для Telegram User (личный аккаунт)
        name: 'Telegram User',
    ...
    {
        id: 'telegram-bot', // ID для Telegram Bot
...
export default function ConnectPlatformPage() {
    ...
    // --- ПРОВЕРКА ПОДПИСКИ И ЗАГРУЗКА ДАННЫХ ---
    useEffect(() => {
        if (!isLoading && user) {
            if (!user.is_premium) {
    ...
    const fetchPlatforms = async () => {
        try {
            ...
            // Загружаем данные о User Telegram и Bot Telegram параллельно
            // Убрал /api из путей, как ты просил
            ...
            // 1. Обработка Telegram User
            if (tgRes.ok) {
                const tgJson = await tgRes.json();
                if (tgJson.platform) setTelegramData(tgJson.platform);
                else setTelegramData(null); // Важно сбросить, если удалили
            }

            // 2. Обработка Telegram Bot
            if (botRes.ok) {
    ...
    const handleCardClick = (platformId: string) => {
        // Логика открытия модалок или редиректа
        if (platformId === 'telegram' && telegramData) {
            setOpenModal('telegram');
        } else if (platformId === 'telegram-bot' && botData) {
            setOpenModal('bot');
        } else {
            // Если платформа НЕ подключена -> идем на страницу подключения
            router.push(`/dashboard/connect/${platformId}`);
        }
    };

    // Удаление бота (передаем в BotDetailsModal)
    const handleDisconnectBot = async () => {
        const token = localStorage.getItem("token");
        try {
            await fetch(`${API_URL}/telegram-bot`, { // Исправлен путь (без /api)
                method: "DELETE",
    ...
    // Callback при закрытии модалки Telegram User (если там произошло отключение)
    const handleCloseTgModal = () => {
        setOpenModal(null);
        // Перезагружаем данные, чтобы убрать бейдж "Connected", если пользователь нажал Disconnect внутри модалки
        fetchPlatforms();
    };
            ...
            <AnimatePresence>
                {openModal === 'telegram' && telegramData && (
                    <PlatformDetailsModal
                        platform={telegramData}
                        onClose={handleCloseTgModal}
                    />
                )}
            ...
            <div className="max-w-5xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {Platforms.map((platform) => {
                        // Определяем статус подключения для конкретной карточки
                        let isConnected = false;

                        // Если это карточка Телеграм И у нас есть данные телеграма
                        if (platform.id === 'telegram' && telegramData) isConnected = true;

                        // Если это карточка Бота И у нас есть данные бота
                        ...
                        return (
                            <motion.div
                                ...
                                className={`
                                    ...
                                    ${isConnected
                                    ? 'border-green-500/30 shadow-[0_0_20px_rgba(34,197,94,0.1)]' // Зеленая подсветка если подключено
                            ...
                            >
                                ...
                                <div className="relative z-10">
                                    ...
                                    <p className="text-sm text-zinc-500 mt-1">
                                        {isConnected
                                            ? <span className="text-zinc-300">Click to manage settings</span>
